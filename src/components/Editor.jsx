...
const RichTextEditor = () => {
    ...
    return (
        <div className={"flex flex-col w-full max-w-2xl text-text"}>
                    ...
                    <div className="flex flex-row border rounded-sm w-fit items-center divide-x">
                        <div className="mr-2">
                            <select name="" id="" className={"px-2 font-light text-sm"}
                                    onChange={(e) => formatText('header', e.target.value)}>
                        ...
                        <div className="flex flex-row px-1">
                            <button onClick={() => formatText('link')} className={"px-3 py-3.5"}><LinkIcon
                                className={"w-4 aspect-square"}/></button>
                            <button onClick={() => attachItems()} className={"px-3 py-3.5"}><PhotoIcon
                                className={"w-4 aspect-square"}/></button>
                        </div>
                        <div className="flex flex-row px-1">
                            <button onClick={() => formatText('align', 'left')} className={"px-3 py-3.5"}>
                                <Bars3BottomLeftIcon className={"w-4 aspect-square"}/></button>
                            <button onClick={() => formatText('align', 'right')} className={"px-3 py-3.5"}>
                                <Bars3BottomRightIcon className={"w-4 aspect-square"}/></button>
                            <button onClick={() => formatText('align', 'center')} className={"px-3 py-3.5"}>
                                <Bars3CenterLeftIcon className={"w-4 aspect-square"}/></button>
                        </div>
                        <div className="flex flex-row px-1">
                            <button onClick={() => formatText('bold', true)}
                                    className={"px-3 py-3.5 text-xs text-center flex justify-center items-center font-bold"}>B
                            </button>
                            <button onClick={() => formatText('italic', true)}
                                    className={"px-3 py-3.5 text-xs text-center flex justify-center items-center font-bold italic"}>I
                            </button>
                        </div>
                        <div className="flex flex-row px-1">
                            <button onClick={() => formatText('list', 'bullet')} className={"px-3 py-3.5"}>
                                <ListBulletIcon className={"w-4 aspect-square"}/></button>
                            <button onClick={() => formatText('list', 'ordered')} className={"px-3 py-3.5"}>
                                <ListBulletIcon className={"w-4 aspect-square"}/></button>
                            <button onClick={() => formatText('removeFormat')} className={"px-3 py-3.5"}>
                                <QueueListIcon className={"w-4 aspect-square"}/></button>
                        </div>
                    </div>
                </div>
                <div className="w-full relative">
                    ...
                    <div className="absolute bottom-0 left-0 z-10 w-full">
                        {embedMenu && (
                            <div className="absolute bg-white -bottom-60 rounded-lg shadow-xl min-w-96 py-2.5">
                                ...
                                <div className="">
                                    <button onClick={() => toggleModal("image")}
                    ...
                    <span className="mt-1 mr-2.5">
                      ...
                      <span className={"ml-1 text-xs"}>Jpeg, PNG</span>
                                    ...
                                    <button onClick={() => toggleModal("youtube")}
                                            className={"flex flex-row py-2 px-2.5 hover:text-gray-100 hover:bg-gray-400 transition-all w-full"}>
                    <span className="mt-1 mr-2.5">
                      ...
                      <span className={"ml-1 text-xs"}>Embed a Youtube Video</span>
                                    ...
                                    <button onClick={() => toggleModal("link")}
                                            className={"flex flex-row py-2 px-2.5 hover:text-gray-100 hover:bg-gray-400 transition-all w-full"}>
                    <span className="mt-1 mr-2.5">
            ...
            <div className="text-right text-gray-600 text-[10px] px-4 py-2 border border-t-0 border-gray-500 mb-4">
                {content.replace(/<\/?[\w\s="/.':;#-\/\?]+>/gi, "").length} words
            </div>

            <button className={"bg-[#0A7227] rounded-lg text-white ml-auto block w-fit py-2 px-4"}>Post
            </button>

            {showModal && (
                <div
                    className="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-black bg-opacity-50">
                    <div className="relative w-full max-w-2xl mx-auto my-6 bg-white rounded-lg shadow-lg">
                            ...
                            modalType === "image" ?
                                <div className="p-4 pt-0 h-full">
                                    ...
                                    <div className="h-full mb-12 max-h-[140px] relative">
                                        ...
                                        <input type="file" className={"absolute border opacity-0 w-full h-full"}/>
                                        <div
                                            className="border border-dashed w-full h-full flex items-center justify-center">
                                            <div
                                                className="border border-[#CEE3D4] w-fit text-xs py-2 px-3 rounded-lg my-14">Import
                                                Image From Device
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex space-x-3 font-medium">
                                        <button className={"bg-[#0A7227] rounded-lg text-white block w-fit py-2 px-4"}
                                                onClick={toggleModal}>Embed
                                        </button>

                                        <button className={"rounded-lg border border-[#CEE3D4] block w-fit py-2 px-4"}
                                ...
                                : modalType === "youtube" ?
                                    <div className="p-4 pt-0 h-full">
                                        <div className="flex flex-col mb-4">
                                            <label className={"mb-2 uppercase text-[10px] font-light"}>Video
                                                Provider</label>
                                            <select value={videoSource}
                                                    className={"text-xs bg-light outline-none border border-primary rounded-lg py-2.5 px-3"}
                                        ...
                                        <div className="flex space-x-3 font-medium">
                                            <button
                                                className={"bg-[#0A7227] rounded-lg text-white block w-fit py-2 px-4"}
                                                onClick={toggleModal}>Embed
                                            </button>

                                            <button
                                                className={"rounded-lg border border-[#CEE3D4] block w-fit py-2 px-4"}
                                                onClick={toggleModal}>Cancel
                                            </button>
                                        </div>
                                    </div>
                                    : modalType === "link" ?
                                        <div className="p-4 pt-0 h-full">
                                            <div className="flex flex-col mb-4">
                                                <label className={"mb-2 uppercase text-[10px] font-light"}>Social Media
                                                    Platform</label>
                                                <select value={videoSource}
                                                        className={"text-xs bg-light outline-none border border-primary rounded-lg py-2.5 px-3"}
                                                        onChange={(e) => setVideoSource(e.target.value)}>
                                            ...
                                            <div className="flex flex-row items-center justify-between mb-4">
                                                ...
                                                <ReactSwitch height={14} width={24} handleDiameter={10}
                                                             checked={disableCaption} checkedIcon={null}
                                                             uncheckedIcon={null} onChange={setDisableCaption}/>
                                            </div>
